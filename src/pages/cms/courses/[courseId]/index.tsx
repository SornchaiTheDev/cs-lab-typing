...
function InCourse() {
  ...
  return (
    <CourseLayout isLoading={course.isLoading} title={course.data?.name as string}>
      <div className="p-4 text-sand-12 md:w-1/2">
        ...
        {course.isLoading ? (
          ...
          <h4 className="text-lg">{course.data?.note as string}</h4>
        )}
        <h5 className="mb-2 mt-4 font-bold">Comment</h5>
        {course.isLoading ? (
          <Skeleton width={"100%"} height={"8rem"} />
        ) : (
          <p>{course.data?.comments as string === "" ? "-" : course.data?.comments as string}</p>
        )}
